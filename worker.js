...
export class PontosGlobaisDO {
  ...
  async adicionarPontosFase(nome, partidaId, faseNumero, pontosTotal, pontosNormal, pontosBonusTotal, categoria) {
    ...
    if (faseExistente) {
      const pontosAntigos = Number(faseExistente.pontosTotal) || 0;
      deltaPontos = pt - pontosAntigos;
    ...
    const existente = this.sql.exec('SELECT pontos FROM jogadores WHERE nome = ? LIMIT 1', canon).toArray()[0] || null;
    const pontosBase = existente ? (Number(existente.pontos) || 0) : 0;
    const novoTotal = Math.max(0, pontosBase + deltaPontos);

    if (!existente) {
      this.sql.exec('INSERT INTO jogadores (nome, pontos, nivel, ultima_partida) VALUES (?, ?, ?, ?)', canon, Math.max(0, pt), 1, now);
      await this.atualizarNivel(canon, Math.max(0, pt));
    } else {
    ...
    const pontosAtual = await this.obterPontos(canon);
    return { nome: canon, pontos: pontosAtual, faseAtualizada: Boolean(faseExistente), deltaPontos };
  }
      ...
      SELECT
        ...
        COALESCE(
          (
            ...
            ORDER BY hf.data DESC, hf.id DESC
            LIMIT 1
          ),
          j.ultima_partida,
          0
        ...
        (
          ...
          ORDER BY hf3.data DESC, hf3.id DESC
          LIMIT 1
        ) AS ultimaFaseNumero
      FROM jogadores j
      ORDER BY j.pontos DESC, ultimaPartidaEm DESC, j.nome ASC
      LIMIT 500
    `).toArray();
  }
